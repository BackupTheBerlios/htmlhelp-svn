...
s/^INSERT INTO `book` (`title`, `default_path`, `default_anchor`) VALUES (\('[^']*'\), \('[^']*'\), \('[^']*'\));$/SELECT @book_id:=`id` from `book` WHERE `title`=\1 LIMIT 1;\
UPDATE `book` SET `default_path`=\2, `default_anchor`=\3;\
